...
## Decision
Generate a local SVG coverage badge in CI using `tj-actions/coverage-badge-py` and commit it to the repository on pushes to `master`.

## Rationale
The action can read the local `.coverage` file produced by the test run and emit an SVG badge, keeping everything self-contained.
