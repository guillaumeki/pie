...
from prototyping_inference_engine.api.atom.set.core.core_algorithm_provider import (
    CoreAlgorithmProvider,
    DefaultCoreAlgorithmProvider,
)
...
class RedundanciesCleanerConjunctiveQuery:
    ...
    def compute_core(self, cq: ConjunctiveQuery) -> ConjunctiveQuery:
        ...
        return ConjunctiveQuery(
            core_atom_set, cq.answer_variables, cq.label, cq.pre_substitution
        )

    def compute_cover(
        self, scq: set[ConjunctiveQuery], del_redundancies_in_cqs: bool = True
        ...
        for cq_to_test in scq:
            ...
            for cq_in_cover in cover:
                if self._cq_query_containment.is_contained_in(cq_to_test, cq_in_cover):
                    add = False
                    break
                elif self._cq_query_containment.is_contained_in(
                    cq_in_cover, cq_to_test
                ):
    ...
    def remove_more_specific_than(
        self, scq1: set[ConjunctiveQuery], scq2: set[ConjunctiveQuery]
    ) -> set[ConjunctiveQuery]:
        new_scq = set()
        for cq1 in scq1:
            if all(
                not self._cq_query_containment.is_contained_in(cq1, cq2) for cq2 in scq2
            ):
                new_scq.add(cq1)
        return new_scq
