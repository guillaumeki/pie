...
class TestNaiveBacktrackHomomorphismAlgorithm(TestCase):
    data: tuple[dict[str, object], ...] = (
        {
            ...
            "homomorphisms": (
                Substitution(
                    {Variable("X"): Constant("a"), Variable("Y"): Constant("b")}
                ),
            ),
        },
        {
            ...
            "homomorphisms": (
                Substitution(
                    {Variable("X"): Constant("a"), Variable("Y"): Constant("a")}
                ),
            ),
        },
        {"from": "p(X,X).", "to": "p(a,b).", "pre_sub": None, "homomorphisms": tuple()},
        {
            "from": "p(X,X), p(X,Y).",
            "to": "p(a,b), p(a,a).",
            "pre_sub": None,
    ...
    def test_exist_homomorphisms(self):
        ...
        for d in self.data:
            ...
            # print(homomorphisms)
            self.assertTrue(
                (len(d["homomorphisms"]) > 0)
                == nbha.exist_homomorphism(from_, to, d["pre_sub"])
            )
