...
class TestNaiveCoreBySpecialization(TestCase):
    data = (
        {"atom_set": "p(X), t(X,a,b), s(a,z), p(Y), t(Y,a,b), relatedTo(Y,z)."},
    ...
    def _is_a_core(atom_set: AtomSet) -> bool:
        for h in NaiveBacktrackHomomorphismAlgorithm.instance().compute_homomorphisms(
            atom_set, atom_set
        ):
    ...
    def _is_equivalent(as1: AtomSet, as2: AtomSet) -> bool:
        return NaiveBacktrackHomomorphismAlgorithm.instance().exist_homomorphism(
            as1, as2
        ) and NaiveBacktrackHomomorphismAlgorithm.instance().exist_homomorphism(
            as2, as1
        )
    ...
    def test_compute_core(self):
        for d in self.data:
            ...
            self.assertTrue(
                self._is_a_core(core), f"result: {core}, atom set: {atom_set}"
            )
            self.assertTrue(
                self._is_equivalent(core, atom_set),
                f"result: {core}, atom set: {atom_set}",
            )
