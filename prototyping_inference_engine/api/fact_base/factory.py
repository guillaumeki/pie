...
class FactBaseFactory:
    ...
    @classmethod
    def create_frozen(cls, atoms: Optional[Iterable[Atom]] = None) -> "FrozenInMemoryFactBase":
        from prototyping_inference_engine.api.fact_base.frozen_in_memory_fact_base import FrozenInMemoryFactBase
        impl = cls._frozen_impl or FrozenInMemoryFactBase
        return impl(atoms)

    @classmethod
    def create_mutable(cls, atoms: Optional[Iterable[Atom]] = None) -> "MutableInMemoryFactBase":
        from prototyping_inference_engine.api.fact_base.mutable_in_memory_fact_base import MutableInMemoryFactBase
        impl = cls._mutable_impl or MutableInMemoryFactBase
        return impl(atoms)

    @classmethod
    def configure(cls, frozen_impl: Optional[Type["FrozenInMemoryFactBase"]] = None,
                  mutable_impl: Optional[Type["MutableInMemoryFactBase"]] = None) -> None:
