"""Semi-oblivious checker: one trigger per frontier image."""
...
class SemiObliviousChecker(TriggerChecker):
    ...
    def check(
        ...
        frontier = Substitution(
            {
                variable: substitution.apply(variable)
                for variable in rule.frontier
                if substitution.apply(variable) != variable
            }
        )
        treated = self._already_treated[rule]
        key = substitution_key(frontier)
        if key in treated:
            return False
        treated.add(key)
        return True
