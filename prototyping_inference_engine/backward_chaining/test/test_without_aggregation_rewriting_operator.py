...
class TestWithoutAggregationRewritingOperator(TestCase):
    data = (
        ...
        {"rules": "g(X) | r(X) :- v(X).", "query": "?() :- g(a) | r(a).", "len_ucq": 1},
        {"rules": "g(X) | r(X) :- v(X).", "query": "?() :- g(a) | r(b).", "len_ucq": 0},
        {
            "rules": "g(X) | r(X) :- v(X).",
            "query": "?() :- g(a) | (r(U), t(U)).",
            "len_ucq": 1,
        ...
        {"rules": "g(X,X) :- v(X).", "query": "?(X,Y) :- g(X,Y).", "len_ucq": 1},
        {
    ...
    def test_rewrite(self):
        for d in self.data:
            ...
            rules = set(DlgpeParser.instance().parse_rules(d["rules"]))
