...
class PieceUnifierAlgorithm:
    ...
    @staticmethod
    def _compute_separating_sticky_variables(
        pre_unifier: PieceUnifier,
    ) -> frozenset[Variable]:
    ...
    def _exists_separating_sticky_variables(pre_unifier: PieceUnifier) -> bool:
        return any(
            v in pre_unifier.separating_variables for v in pre_unifier.sticky_variables
        )
                ...
                if a.predicate == b.predicate:
                    ...
                    if by_position_part.is_valid(rule, query):
                        atomic_pre_unifiers.append(
                            PieceUnifier(
                                rule, query, FrozenAtomSet([b]), by_position_part
                            )
                        )
        return atomic_pre_unifiers

    @staticmethod
    def _compute_var_to_query_atoms(
    ...
    @staticmethod
    def _compute_atom_to_atomic_pre_unifiers(
        apu: list[PieceUnifier],
    ) -> dict[Atom, set[PieceUnifier]]:
    ...
    @staticmethod
    def _extend_atomic_pre_unifiers(
        query: ConjunctiveQuery, apu: list[PieceUnifier]
    ) -> list[PieceUnifier]:
        ...
        while to_extend:
            ...
            if len(pre_unifier.unified_query_part) == 1:
                atom_to_apu[next(iter(pre_unifier.unified_query_part))].remove(
                    pre_unifier
                )
            neighbour_atoms = {
                a
                for sep in pre_unifier.sticky_variables
                for a in var_to_query_atoms[sep]
                if a not in pre_unifier.unified_query_part
            }
            for extended_pu in PieceUnifierAlgorithm._compute_local_extension(
                pre_unifier, neighbour_atoms, atom_to_apu
            ):
                if PieceUnifierAlgorithm._exists_separating_sticky_variables(
                    extended_pu
                ):
    ...
    @staticmethod
    def _compute_local_extension(
        pu: PieceUnifier,
        neighbour_atoms: set[Atom],
        atom_to_apu: dict[Atom, set[PieceUnifier]],
    ) -> list[PieceUnifier]:
