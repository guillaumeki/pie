...
class TestDlgp2Parser(TestCase):
    def test_parse_atoms(self):
        ...
        atoms = Dlgp2Parser.instance().parse_atoms(atoms_str)
    ...
    def test_parse_union_conjunctive_queries(self):
                ...
                "ucqs": {
                    UnionQuery(
                        [
                            ConjunctiveQuery(
                                FrozenAtomSet(
                                    Dlgp2Parser.instance().parse_atoms("q(U), r(U).")
                                )
                            )
                        ]
                    )
                ...
                "ucqs": {
                    UnionQuery(
                        [
                            ConjunctiveQuery(
                                FrozenAtomSet(
                                    Dlgp2Parser.instance().parse_atoms(
                                        "g(U), e(U,V), g(V)."
                                    )
                                )
                            ),
        ...
        for test in tests:
            ucq = set(
                Dlgp2Parser.instance().parse_union_conjunctive_queries(test["to_parse"])
            )
