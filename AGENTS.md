...
- The documentation page for algorithm references must be structured by concept/algorithm (not filenames), use proper prose (no raw code-comment dumps), include longer explanations, and provide clickable links to publications. Author names must link to personal pages when available, otherwise to Google Scholar, otherwise to DBLP.
